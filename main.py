...
st.markdown("""
    ...
    * **Kein Riba (Zinsen):** In Krisen schichten wir in physisches Gold umschalten, statt in verzinstes Sicherungsvermögen.
...
MONATLICHER_SPARPLAN = st.sidebar.number_input("Monatlicher Sparplan ($)", min_value=0, max_value=10000, value=250,
                                               step=50)
ORDER_GEBUEHR = st.sidebar.slider("Gebühr pro Umschichtung ($)", min_value=0.0, max_value=20.0, value=4.90, step=0.10)
SMA_FENSTER = st.sidebar.slider("Trend-Filter (Tage des gleitenden Durchschnitts)", min_value=50, max_value=250,
                                value=200, step=10)
...
if df.empty:
    ...
    col1.metric(label="🏆 Endwert invest2gether Halal-Bot", value=f"{endwert_bot:,.2f} $",
                delta=f"Eingezahlt: {eingezahltes_kapital:,.2f} $")
    col2.metric(label="📉 Endwert Klassischer Halal-Markt", value=f"{endwert_bh:,.2f} $", delta=f"Differenz zum Bot",
                delta_color="inverse")
    col3.metric(label="💰 Ihr krisengeschützter Mehrwert", value=f"{mehrwert:,.2f} $",
                delta="Mehrwert durch KI-Umschichtung")
    ...
    with zakat_col:
        ...
        st.caption(
            "Entspricht den verpflichtenden 2,5 % Abgabe auf Ihr ruhendes Vermögen. Diese Reinigung sichert den Segen (*Barakah*) Ihres Kapitals.")

    with purif_col:
        st.info(f"📌 **Empfohlene Spenden-Reinigung: {purification_betrag:,.2f} $**")
        st.caption(
            "Berechnete 0,05 % Reinigungssumme für unvermeidbare minimale Zinserträge der im Welt-ETF enthaltenen Großkonzerne.")
    ...
    ax.plot(df.index, df['Bot_Depotwert'], label='invest2gether Halal-Bot (Aktien/Gold)', color='#D4AF37',
            linewidth=2.5)
    ...
    # ==============================================================================
    # INTERAKTIVE WARTELISTE (Smoke-Test für Investoren)
    # ==============================================================================
    st.markdown("---")
    st.subheader("🚀 Werden Sie Teil der Beta-Phase von invest2gether")
    st.markdown(
        "Haben wir Ihr Interesse geweckt? Tragen Sie sich unverbindlich auf unsere Warteliste ein, um den offiziellen Start nicht zu verpassen.")
    ...
    with st.form(key='waitlist_form', clear_on_submit=True):
        ...
        if submit_button:
            if kunden_name and kunden_email:
                ...
                st.balloons()
                st.success(
                    f"Salam {kunden_name}! Vielen Dank für Ihr Vertrauen. Sie wurden erfolgreich auf der invest2gether-Warteliste eingetragen.")
            else:
